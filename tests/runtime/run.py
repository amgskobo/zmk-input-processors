...
source_dir = root / "drivers/input"
links = ()
# Sources with a coverage gate of their own.
elsewhere = ()
fixtures = (
    ("code_mapper", "input_processor_runtime_code_mapper.c", (
    ...
    ("scaler", "input_processor_runtime_scaler.c", (
        "encode_ratio",
        "decode_ratio",
        "runtime_scaler_get_params",
        "runtime_scaler_set_params",
        "scale_value",
        "runtime_scaler_handle_event",
        "runtime_scaler_init",
        "runtime_scaler_device_valid",
    ), ()),
    ("transform", "input_processor_runtime_transform.c", (
        "encode_flags",
        "decode_flags",
    ...
    ("temp_layer", "input_processor_runtime_temp_layer.c", (
        "set_layer_locked",
        "activate_work_cb",
        "deactivate_work_cb",
        "runtime_temp_layer_get_params",
        "runtime_temp_layer_set_params",
        "position_is_excluded",
        "handle_layer_state_changed",
        "handle_position_state_changed",
        "handle_keycode_state_changed",
        "dispatch",
        "runtime_temp_layer_handle_event",
        "runtime_temp_layer_init",
        "runtime_temp_layer_device_valid",
        "#define DISPATCH_TO_INST",
        "runtime_temp_layer_event_cb",
    ), ()),
    ("custom_settings", "input_processors_custom_settings.c", (
        "input_processors_namespace_handler",
        "input_processors_read_bool",
        "input_processors_read_uint32",
        "input_processors_check_unique_keys",
    ), ()),
    # Each processor's bridge from its settings to its driver.
    ("code_mapper_settings", "input_processor_runtime_code_mapper_custom_settings.c", (
        "#define RUNTIME_CODE_MAPPER_APPLY",
        "runtime_code_mapper_apply_settings",
        "runtime_code_mapper_settings_event_cb",
    ), ()),
    ("scaler_settings", "input_processor_runtime_scaler_custom_settings.c", (
        "#define RUNTIME_SCALER_APPLY",
        "runtime_scaler_apply_settings",
        "runtime_scaler_settings_event_cb",
    ), ()),
    ("transform_settings", "input_processor_runtime_transform_custom_settings.c", (
        "#define RUNTIME_TRANSFORM_APPLY",
        "runtime_transform_apply_settings",
        "runtime_transform_settings_event_cb",
    ), ()),
    ("temp_layer_settings", "input_processor_runtime_temp_layer_custom_settings.c", (
        "#define RUNTIME_TEMP_LAYER_APPLY",
        "runtime_temp_layer_apply_settings",
        "runtime_temp_layer_settings_event_cb",
    ), ()),
)
